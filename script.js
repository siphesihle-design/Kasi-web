document.addEventListener('DOMContentLoaded', () => {
  ...
  async function getRole(uid) {
    try {
      ...
      if (snap.exists()) return snap.val().role || 'client';
  ...
  function initBookingModal(user) {
    ...
    form?.addEventListener('submit', async (e) => {
      ...
      // Extract price from service string "Haircut R150"
      ...
      const booking = {
        ...
        time,
        service,
        price, // <-- store price separately for revenue calc
        salon: selectedSalon,
  ...
  // 10. Redirect helper - prevents reload loops
  function redirectByRole(role) {
    const page = window.location.pathname.split('/').pop();
    if (role === 'admin' && page!== 'admin.html') window.location.href = 'admin.html';
    else if (role === 'salon_owner' && page!== 'owners.html') window.location.href = 'owners.html';
    else if (role === 'client' && page!== 'salons.html' && page!== '') window.location.href = 'salons.html';
  }
});
